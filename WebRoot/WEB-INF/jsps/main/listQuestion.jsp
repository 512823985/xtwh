        ...
        <td colspan="8">
        	<div class="pagelist"> 
        		<c:if test="${pageInfo.hasPreviousPage }">
	        		<a href="${pageContext.request.contextPath }/question/list.do?page=${pageInfo.pageNum-1}&code=${code}">上一页</a> 
        		</c:if>
        		...
        		<c:forEach var="idx" items="${pageInfo.navigatepageNums }">
        			<c:choose>
		        		...
		        		<c:otherwise>
		        			<a href="${pageContext.request.contextPath }/question/list.do?page=${idx}&code=${code}">${idx }</a>
		        		</c:otherwise>
	        		</c:choose>
        		</c:forEach>
        		
        		<c:if test="${pageInfo.hasNextPage }">
        			<a href="${pageContext.request.contextPath }/question/list.do?page=${pageInfo.pageNum+1}&code=${code}">下一页</a>
        		</c:if>
        	</div>
        </td>
      </tr>
		...
		if(confirm("您确定要删除吗?")){
			$.ajax({
				...
				success:function(msg) {
					alert(msg);
					window.location="${pageContext.request.contextPath}/question/list.do?code=${code}";
				}
			});
		}
	} else {
